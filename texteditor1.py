import tkinter as tk
from tkinter import ttk
from tkinter import font,colorchooser,filedialog
import os
from tkinter import messagebox


txt_editor=tk.Tk()
txt_editor.geometry("1250x600")
txt_editor.title("Jha text editor")



#------------------------------------MAIN MENU----------------------------------------------

main_menu_editor=tk.Menu()

new_icon=tk.PhotoImage(file="icons2/new.png")
open_icon=tk.PhotoImage(file="icons2/open.png")
save_icon=tk.PhotoImage(file="icons2/save.png")
save_as_icon=tk.PhotoImage(file="icons2/save_as.png")
exit_icon=tk.PhotoImage(file="icons2/exit.png")


#icons------------------------------------------------#







file=tk.Menu(main_menu_editor,tearoff=False)
#--------------------------------------For Files-------------------------------------------

copy_icon=tk.PhotoImage(file="icons2/copy.png")
paste_icon=tk.PhotoImage(file="icons2/paste.png")
cut_icon=tk.PhotoImage(file="icons2/cut.png")
clear_all_icon=tk.PhotoImage(file="icons2/clear_all.png")
find_icon=tk.PhotoImage(file="icons2/find.png")

edit=tk.Menu(main_menu_editor,tearoff=False)

#-------------------------------------Edit---------------------------------------------------




tool_bar_icon=tk.PhotoImage(file="icons2/tool_bar.png")
status_bar_icon=tk.PhotoImage(file="icons2/status_bar.png")

view=tk.Menu(main_menu_editor,tearoff=False)
#----------------------------------------view-------------------------------------------


light_default_icon=tk.PhotoImage(file="icons2/light_default.png")
light_plus_icon=tk.PhotoImage(file="icons2/light_plus.png")
dark_icon=tk.PhotoImage(file="icons2/dark.png")
red_icon=tk.PhotoImage(file="icons2/red.png")
monokai_icon=tk.PhotoImage(file="icons2/monokai.png")
night_blue_icon=tk.PhotoImage(file="icons2/night_blue.png")


color=tk.Menu(main_menu_editor,tearoff=False)

#-----------------------------------------color-------------------------------------------

theme_choise=tk.StringVar()
color_icons=(light_default_icon,light_plus_icon,dark_icon,red_icon,monokai_icon,night_blue_icon)

color_dict={
    'Light Default':('#000000','#ffffff'),
    'Light Plus':('#474747','#e0e0e0'),
    'Dark':('#c4c4c4','#2d2d2d'),
    'Red':('#2d2d2d','#ffe8e8'),
    'Monakai':('#d3b774','#474747'),
    'Night Blue':('#ededed','#6b9dc2'),
}



#cascade-------------------

main_menu_editor.add_cascade(label="File",menu=file)
main_menu_editor.add_cascade(label="Edit",menu=edit)
main_menu_editor.add_cascade(label="View",menu=view)
main_menu_editor.add_cascade(label="Color",menu=color)


#----------------------------------------Commands Area-----------------------------------------#
#------------------------------File-------------------------------------------
#------variable for file--------
url=''

#NEW----------


def new_file(event=None):
    global url
    url=''
    editor.delete(1.0,tk.END)

#-----------------------------------OPEN----------------------------------

def open_file(event=None):
    global url
    url=filedialog.askopenfilename(initialdir=os.getcwd(), title='Select File',filetypes=(('Text File', '*.txt'),('All files','*.*')))
    try:
        with open(url , 'r')as fr:
            editor.delete(1.0,tk.END)
            editor.insert(1.,fr.read())

    except FileNotFoundError:
        return
    except:
        return

    txt_editor.title(os.path.basename(url))




#save-------------------------------------------------------------------

def save_file(event=None):
    global url
    try:
        if url:
            content=str(editor.get(1.0,tk.END))
            with open(url , 'w' , encoding='utf-8')as f:
                f.write(content)
        else:
            url= filedialog.asksaveasfile(mode='w',defaultextension='.txt',filetypes=(('Text File', '*.txt'),('All files','*.*')))
            content2=editor.get(1.0,tk.END)
            url.write(content2)
            url.close()
    except:
        return

#-----------------------------save as---------------------------------

def save_as_file(event=None):
    global url
    try:
        content=editor.get(1.0,tk.END)
        url= filedialog.asksaveasfile(mode='w',defaultextension='.txt',filetypes=(('Text File', '*.txt'),('All files','*.*')))
        
        url.write(content)
        url.close()

    except:
        return



#exit-----------------------------------------------------


def exit_file(event=None):
    global url,text_changed
    try:
        if text_changed:
            mbox=messagebox.askyesnocancel('Warning','Do you want to save a file ?')
            if mbox is True:
                if url:
                    content=editor.get(1.0,tk.END)
                    with open(url , 'w' , encoding='utf-8')as f:
                        f.write(content)
                        txt_editor.destroy()

                else:
                    content2=editor.get(1.0,tk.END)
                    url=filedialog.asksaveasfile(mode='w',defaultextension='.txt',filetypes=(('Text File', '*.txt'),('All files','*.*')))

                    url.write(content2)
                    url.close()
                    txt_editor.destroy()

            elif mbox is False:
                txt_editor.destroy()

        else:
            txt_editor.destroy()

    except:
        return









file.add_command(label='New',image=new_icon,compound=tk.LEFT,accelerator='Ctrl+N',command=new_file)
file.add_command(label='Open',image=open_icon,compound=tk.LEFT,accelerator='Ctrl+O',command=open_file)
file.add_command(label='Save',image=save_icon,compound=tk.LEFT,accelerator='Ctrl+S',command=save_file)
file.add_command(label='Save As',image=save_as_icon,compound=tk.LEFT,accelerator='Ctrl+Alt+S',command=save_as_file)
file.add_command(label='Exit',image=exit_icon,compound=tk.LEFT,accelerator='Ctrl+Q',command=exit_file)

#------------------------------Edit-------------------------------------------
def find_file(event=None):
    def find():
        word=find_input.get()
        editor.tag_remove('match','1.0',tk.END)
        matches=0
        if word:
            start_pos='1.0'
            while True:
                start_pos=editor.search(word,start_pos,stopindex=tk.END)
                if not start_pos:
                    break
                end_pos=f"{start_pos}+{len(word)}c"
                editor.tag_add('match',start_pos,end_pos)
                matches += 1
                start_pos=end_pos
                editor.tag_config('match',foreground='red',background='yellow')


        
    def replace():
        word=find_input.get()
        replace_txt=replace_input.get()
        content=editor.get(1.0,tk.END)
        new_content=content.replace(word,replace_txt)
        editor.delete(1.0,tk.END)
        editor.insert(1.0,new_content)
        
    

    find_di=tk.Toplevel()
    find_di.geometry('450x250+500+200')
    find_di.title('Find')
    find_di.resizable(0,0)

#frame for find

    find_frame=ttk.LabelFrame(find_di ,text='Find/Replace')
    find_frame.pack(pady=20)



    #label


    text_find_label=ttk.Label(find_frame,text='Find :')
    text_find_label.grid(row=0,column=0,padx=4,pady=4)
    text_replace_label=ttk.Label(find_frame,text='Replace :')
    text_replace_label.grid(row=1,column=0,padx=4,pady=4)


    #entry


    find_input=ttk.Entry(find_frame,width=30)
    find_input.grid(row=0,column=1,padx=4,pady=4)
    replace_input=ttk.Entry(find_frame,width=30)
    replace_input.grid(row=1,column=1,padx=4,pady=4)


    #buttun

    find_btn=ttk.Button(find_frame,text='Find',command=find)
    find_btn.grid(row=2,column=0,padx=8,pady=4)
    replace_btn=ttk.Button(find_frame,text='Replace',command=replace)
    replace_btn.grid(row=2,column=1,padx=8,pady=4)

    find_di.mainloop()














edit.add_command(label='Copy',image=copy_icon,compound=tk.LEFT,accelerator='Ctrl+C',command=lambda :editor.event_generate("<Control c>"))
edit.add_command(label='Paste',image=paste_icon,compound=tk.LEFT,accelerator='Ctrl+V',command=lambda :editor.event_generate("<Control v>"))
edit.add_command(label='Cut',image=cut_icon,compound=tk.LEFT,accelerator='Ctrl+X',command=lambda :editor.event_generate("<Control x>"))
edit.add_command(label='Clear All',image=clear_all_icon,compound=tk.LEFT,accelerator='Ctrl+Alt+K',command=lambda :editor.delete(1.0,tk.END))
edit.add_command(label='Find',image=find_icon,compound=tk.LEFT,accelerator='Ctrl+P',command=find_file)
#------------------------------view-------------------------------------------
show_statusbar=tk.BooleanVar()
show_statusbar.set(True)
show_toolbar=tk.BooleanVar()
show_toolbar.set(True)




def hide_toolbar():
    global show_toolbar
    if show_toolbar:
        tool_bar.pack_forget()
        show_toolbar=False
    else:
        editor.pack_forget()
        status_bar.pack_forget()
        tool_bar.pack(side=tk.TOP,fill=tk.X)
        editor.pack(fill=tk.BOTH,expand=True)
        status_bar.pack(side=tk.BOTTOM)
        show_toolbar=True

def hide_statusbar():
    global show_statusbar
    if show_statusbar:
        status_bar.pack_forget()
        show_statusbar=False
    else:
        status_bar.pack(side=tk.BOTTOM)
        show_statusbar=True
view.add_checkbutton(label='Tool Bar',onvalue=True,offvalue=0,variable=show_toolbar,image=tool_bar_icon,compound=tk.LEFT,accelerator='Ctrl+T',command=hide_toolbar)
view.add_checkbutton(label='Status Bar',onvalue=1,offvalue=False,variable=show_statusbar,image=status_bar_icon,compound=tk.LEFT,accelerator='Ctrl+S+T',command=hide_statusbar)

#------------------------------Color-------------------------------------------
 #---------------------------------toolbar--------------------------------------

 #font box----------------------------------------------------------------
tool_bar=ttk.Label(txt_editor)
tool_bar.pack(side=tk.TOP,fill=tk.X)
font_tuples=tk.font.families()
font_family=tk.StringVar()
font_box=ttk.Combobox(tool_bar,width=30,textvariable=font_family,state='readonly')  
font_box['values']=font_tuples
font_box.current(font_tuples.index("Arial"))
font_box.grid(row=0,column=0,padx=1)





#size box-------------------------------------------------------------------------


size_var=tk.IntVar()
font_size=ttk.Combobox(tool_bar,width=14,textvariable=size_var,state="readonly")
font_size['values']=tuple(range(8,80,2))
font_size.current(4)
font_size.grid(row=0,column=1,padx=3)


#bold icon-------------------------------------------------------------------------

bold_icon=tk.PhotoImage(file='icons2/bold.png')
bold_btn=ttk.Button(tool_bar,image=bold_icon)
bold_btn.grid(row=0,column=3,padx=3)


italic_icon=tk.PhotoImage(file='icons2/italic.png')

italic_btn=ttk.Button(tool_bar,image=italic_icon)
italic_btn.grid(row=0,column=4,padx=3)


underline_icon=tk.PhotoImage(file='icons2/underline.png')

underline_btn=ttk.Button(tool_bar,image=underline_icon)
underline_btn.grid(row=0,column=5,padx=3)

#------------------------------------------color font------------------------------
font_color_icon=tk.PhotoImage(file='icons2/font_color.png')
font_color_btn=ttk.Button(tool_bar,image=font_color_icon)
font_color_btn.grid(row=0,column=6,padx=3)


#alignLeft________---------------------------------------------------------

align_left_icon=tk.PhotoImage(file='icons2/align_left.png')
align_left_btn=ttk.Button(tool_bar,image=align_left_icon)
align_left_btn.grid(row=0,column=7,padx=3)




align_center_icon=tk.PhotoImage(file='icons2/align_center.png')
align_center_btn=ttk.Button(tool_bar,image=align_center_icon)
align_center_btn.grid(row=0,column=8,padx=3)




align_right_icon=tk.PhotoImage(file='icons2/align_right.png')
align_right_btn=ttk.Button(tool_bar,image=align_right_icon)
align_right_btn.grid(row=0,column=9,padx=3)







#---------------------------------------------------editor_______________________________

editor=tk.Text(txt_editor)
editor.config(wrap='word',relief=tk.FLAT)

scroll_bar=tk.Scrollbar(txt_editor)
editor.focus_set()
scroll_bar.pack(side=tk.RIGHT,fill=tk.Y)
editor.pack(fill=tk.BOTH,expand=True)
scroll_bar.config(command=editor.yview)
editor.config(yscrollcommand=scroll_bar.set)


#functionality--------------font family,and font size

#family
current_font_family='Ariel'
current_font_size=12


def change_font(txt_editor):
    global current_font_family
    current_font_family=font_family.get()
    editor.configure(font=(current_font_family,current_font_size))

font_box.bind("<<ComboboxSelected>>",change_font)


#size

def change_font_size(txt_editor):
    global current_font_size
    current_font_size=size_var.get()
    editor.configure(font=(current_font_family,current_font_size))

font_size.bind("<<ComboboxSelected>>",change_font_size)

font_box.bind("<<ComboboxSelected>>",change_font)
editor.configure(font=('Ariel',12))


#button Functionality--------------------------------------------------------------------


#bold------


def change_bold():
    text_property=tk.font.Font(font=editor['font'])
    if text_property.actual()['weight'] == 'normal':
        editor.configure(font=(current_font_family,current_font_size,'bold'))
    if text_property.actual()['weight'] == 'bold':
        editor.configure(font=(current_font_family,current_font_size,'normal'))

bold_btn.configure(command=change_bold)



def change_italic():
    text_property=tk.font.Font(font=editor['font'])
    if text_property.actual()['slant'] == 'roman':
        editor.configure(font=(current_font_family,current_font_size,'italic'))
    if text_property.actual()['slant'] == 'italic':
        editor.configure(font=(current_font_family,current_font_size,'roman'))

italic_btn.configure(command=change_italic)



def change_underline():
    text_property=tk.font.Font(font=editor['font'])
    if text_property.actual()['underline'] == 0:
        editor.configure(font=(current_font_family,current_font_size,'underline'))
    if text_property.actual()['underline'] == 'underline':
        editor.configure(font=(current_font_family,current_font_size,0))

underline_btn.configure(command=change_underline)








#color functationality __--------------------------------------

def change_font_color():
    color_var=tk.colorchooser.askcolor()
    editor.configure(fg=color_var[1])

font_color_btn.configure(command=change_font_color)



#align functionality-----------------------------------------------------------------

def align_left():
    text_content=editor.get(1.0,'end')
    editor.tag_config('left',justify=tk.LEFT)
    editor.delete(1.0,tk.END)
    editor.insert(tk.INSERT,text_content,'left')

align_left_btn.configure(command=align_left)



def align_center():
    text_content=editor.get(1.0,'end')
    editor.tag_config('center',justify=tk.CENTER)
    editor.delete(1.0,tk.END)
    editor.insert(tk.INSERT,text_content,'center')

align_center_btn.configure(command=align_center)



def align_right():
    text_content=editor.get(1.0,'end')
    editor.tag_config('right',justify=tk.RIGHT)
    editor.delete(1.0,tk.END)
    editor.insert(tk.INSERT,text_content,'right')

align_right_btn.configure(command=align_right)





editor.configure(font=('Ariel',12))
     





#status Bar--------------

status_bar=ttk.Label(txt_editor,text="Status Bar")
status_bar.pack(side=tk.BOTTOM)

text_changed=True

def changed(event=None):
    if editor.edit_modified():
        text_changed=True
        words=len(editor.get(1.0, 'end-1c').split())
        characters=len(editor.get(1.0 , 'end-1c').replace(' ',''))
        status_bar.config(text=f"Characters : {characters} Words:{words}")
    editor.edit_modified(False)
editor.bind("<<Modified>>",changed)



#------NEW--------
def change_theme():
    chosen_theme=theme_choise.get()
    color_tuple=color_dict.get(chosen_theme)
    fg_color,bg_color=color_tuple[0],color_tuple[1]
    editor.config(bg=bg_color,fg=fg_color)




count=0
for i in color_dict:
    color.add_radiobutton(label=i,image=color_icons[count],variable=theme_choise,compound=tk.LEFT,command=change_theme)
    count+=1

txt_editor.bind("<Control-n>",new_file)
txt_editor.bind("<Control-s>",save_file)
txt_editor.bind("<Control-o>",open_file)
txt_editor.bind("<Control-q>",exit_file)
txt_editor.bind("<Control-Alt-s>",save_as_file)
txt_editor.bind("<Control-f>",find_file)
txt_editor.bind(lambda :editor.event_generate("<Control x>"))
txt_editor.bind(lambda :editor.event_generate("<Control c>"))
txt_editor.bind(lambda :editor.event_generate("<Control v>"))
txt_editor.bind(lambda :editor.delete(1.0,tk.END))

txt_editor.config(menu=main_menu_editor)
txt_editor.mainloop()